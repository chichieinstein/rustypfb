...
if __name__ == "__main__":
    ...
    ax[0].plot(2.5 - 5.0 * np.array([ind for ind in range(100, 510)][::-1]).astype("float32") / 1024, reduced_iq[100:510][::-1])
    ...
    f, psd = sig.welch(iq, fs=5.0, window=("kaiser", 10.0), nperseg=Nwind, noverlap=Nover, nfft=Nfft, return_onesided=False)
    fsp, t, spec = sig.spectrogram(iq, fs=5.0, window=("kaiser", 10.0), nperseg=Nwind, noverlap=Nover, nfft=Nfft, return_onesided=False)
    ...
    new_ax[0].plot(np.fft.fftshift(f)[400:650], reduced_psd[::-1][400:650])
    # new_ax[0].axes.set_aspect('auto')
    # new_ax[0].axes.set_ylabel("sample count")
    new_ax[0].axes.set_aspect('auto')
    new_ax[0].axes.set_xlabel("Frequency (MHz)")
    new_ax[0].axes.set_ylabel("Power in dB")
    new_ax[0].axes.set_ylim(-60.0, 0)
    new_ax[0].axes.grid(axis="y", which="major")
